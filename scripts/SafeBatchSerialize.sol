...
contract SafeTxUtil is Script {
    ...
    function logTxsJson(SerializedTx[] memory txs, string memory path) public {
        string memory json = _buildTxsJson(txs, "0");
        console.log("============================================================");
    ...
    function _buildTxsJson(SerializedTx[] memory txs, string memory operation) internal view returns (string memory) {
        // transactions array
    ...
    /// @param operation Safe operation for every tx in the batch: "0" = CALL, "1" = DELEGATECALL
    ///        (a SafeDelegateBatch `execute()` payload).
    function writeTxs(SerializedTx[] memory txs, string memory path, string memory operation) public {
        ...
        if (vm.envOr("SAFE_TX_CONSOLE_JSON", false)) {
            _logTxsJson(txs, path, operation);
            return;
        }
        ...
        for (uint i = 0; i < txs.length; i++) {
            ...
            vm.serializeString(transaction, "operation", operation);
    ...
    function _logTxsJson(SerializedTx[] memory txs, string memory path, string memory operation) internal view {
